...
# from vnpy.trader.gateway.secGateway.secGateway import SecMdApi, SecTdApi
...
class SecGateway(VtGateway):
    ...
    def __init__(self, eventEngine, gatewayName='SEC'):
        ...
        print('Runing sec-gateway modify-version')
    #----------------------------------------------------------------------
    def connect(self):
        """连接"""       
        try:
        ...
        # 创建行情和交易接口对象
        if setting.get('mdPassword') is not None:
            mdPassword = str(setting['mdPassword'])
            print(accountID, mdPassword, mdAddress)
            self.mdApi.connect(accountID, mdPassword, mdAddress)
        else:
            print(accountID, password, mdAddress)
